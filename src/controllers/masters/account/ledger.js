...
const createLedger = async (req, res) => {
  try {
    ...
    const ledger = await DefaultLedgerService.createCustomLedger(
      ledgerData,
      req.user.id
    );

    const createdLedger = await DefaultLedgerService.getLedgerWithDefaultInfo(
      ledger.id
    );
  ...
  try {
    ...
    const ledgersWithInfo = await Promise.all(
      rows.map(async (ledger) => {
        const editableFields = await DefaultLedgerService.getEditableFields(
          ledger.id
        );
...
const getLedgerByCompanyId = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 10,
...
const getLedgerById = async (req, res) => {
  try {
    const { id } = req.params;
    const companyId = req.companyId;
    const ledger = await DefaultLedgerService.getLedgerWithDefaultInfo(
      id,
      companyId
    );
...
const updateLedger = async (req, res) => {
  try {
    ...
    if (
      updateData.balanceType &&
      !["Debit", "Credit"].includes(updateData.balanceType)
    ) {
      return res.status(400).json({
        success: false,
        message: "Invalid balance type. Must be Debit or Credit",
    ...
    if (updateData.ledgerName)
      updateData.ledgerName = updateData.ledgerName.trim();
    ...
    await DefaultLedgerService.updateDefaultLedger(
      id,
      updateData,
      req.user.id,
      req.companyId
    ...
    const updatedLedger = await DefaultLedgerService.getLedgerWithDefaultInfo(
      id,
      req.companyId
    );
...
const deleteLedger = async (req, res) => {
  try {
    ...
    const transactionCount = await Transaction.count({
      where: {
        [Op.or]: [{ debitLedgerId: id }, { creditLedgerId: id }],
      },
    });
    ...
    await DefaultLedgerService.deleteLedger(id, req.user.id, req.companyId);
...
const getLedgerTransactions = async (req, res) => {
  try {
    const { id } = req.params;
    const { page = 1, limit, startDate, endDate, voucherType } = req.query;
...
const getLedgerDetails = async (req, res) => {
  try {
    ...
    const totalDebitAmount = ledgerEntries.reduce(
      (sum, entry) => sum + entry.debit,
      0
    );
    const totalCreditAmount = ledgerEntries.reduce(
  ...
  try {
    ...
    if (transactionCount > 0) {
      ...
      return res.status(400).json({
        success: false,
        message:
          "Cannot update opening balance. Ledger has posted transactions.",
      });
    }
...
const getDefaultLedgers = async (req, res) => {
  try {
    ...
    let defaultLedgers;
    if (groupId) {
      defaultLedgers = await DefaultLedgerService.getDefaultLedgersByGroup(
        groupId
      );
    } else {
